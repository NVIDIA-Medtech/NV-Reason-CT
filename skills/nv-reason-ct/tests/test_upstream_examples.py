...
def _run_logged(command, output_dir, name):
    ...
    assert (
        result.returncode == 0
    ), f"{name}: see {output_dir} (exit {result.returncode})"
...
def test_live_example_matches_upstream(
    live_revision, upstream_checkout, tmp_path, region
):
    ...
    prompt = (
        f"write a structured {'abdominal' if region == 'abdomen' else region} CT report"
    )
